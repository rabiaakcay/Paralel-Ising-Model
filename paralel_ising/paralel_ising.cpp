...
//void write_initial(bool spins[SIZE]);
...
int main(void)
{
   ...
   // clock_t baslangic = clock(), bitis;
	...
	int neighs[SIZE][4]; //To store nearest neighbours, [row][column]

    mt19937 gen(958431198); //Mersenne Twister RNG
	uniform_int_distribution<int> brandom(0, 1); //Get any random integer, 0 for spin down and 1 for spin up
	uniform_int_distribution<int> ran_pos(0, SIZE-1); //Get any random integer, to scan all the lattice
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    double start_initialize = omp_get_wtime();
    initialize(spins, gen, brandom); //Init randomly
    double finish_initialize = omp_get_wtime(); 
    cout << "Time is initialize: " << finish_initialize-start_initialize <<endl;
    
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////   
    double start_get_neighbors = omp_get_wtime();
	get_neighbors(neighs); //Get neighbour table
	double finish_get_neighbors = omp_get_wtime(); 
    cout << "Time is get_neighbors: "<<finish_get_neighbors - start_get_neighbors << endl;
    
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	double start_get_energy = omp_get_wtime();
	energy = get_energy(spins, neighs); //Compute initial energy
	double finish_get_energy = omp_get_wtime(); 
    cout << "Time is get_energy: "<<finish_get_energy - start_get_energy << endl;
    
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  

    N = 1e3;  // Averaging the system N times

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	double start_write = omp_get_wtime();
   //write_initial(spins);
    write(spins); // Display the spin configuration at Tc 
    double finish_write = omp_get_wtime(); 
    cout << "Time is write: "<<finish_write - start_write << endl;
    
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  

	output.open("test_alt_50_1000.txt");
	for (tstar = tmax; tstar > tcrit_up; tstar -= deltat)
	{
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
        double start_do_step = omp_get_wtime();
	    do_step(spins, neighs, tstar, N, h, energy, gen, ran_u, ran_pos, m);
	    double finish_do_step= omp_get_wtime(); 
        cout << "Time is do_step: "<<finish_do_step - start_do_step << endl;
    
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
 		double start_w_output = omp_get_wtime();
	    w_output(output, tstar, N, m);
	    double finish_w_output= omp_get_wtime(); 
        cout << "Time is w_output: "<<finish_w_output - start_w_output << endl;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	...
	{
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	  /* 	double start_do_step_wolff = omp_get_wtime();
	    do_step_wolff(spins, neighs, tstar, N, energy, gen, ran_u, ran_pos, m);
	    double finish_do_step_wolff= omp_get_wtime(); 
        cout << "Time is do_step_wolff: "<<finish_do_step_wolff - start_do_step_wolff << endl;*/
...
void write_initial(bool spins[SIZE])
{
    ...
    output_initial.open("check_initial.txt");
    for (i=0; i < N; i++)
    {
        if (i % SIZE == 0 and i != 0) output_initial << endl;
        output_initial << spins[i] << " ";
    }
...
void get_neighbors(int neighs[SIZE][4])
{
    ...
    #pragma omp parallel for private(i,j,u,d,r,l) collapse(2)
	for (i=0; i < L; i++)
	{
		for (j=0; j < L; j++)
		{
		    //Get the (x,y) with periodic boundaries. Here we use Conditional ternary operator ( ? )
			u = j+1 == L ? 0 : j+1; // condition ? result1 : result2
			d = j-1 == -1 ? L-1 : j-1;
			r = i+1 == L ? 0 : i+1;
			l = i-1 == -1 ? L-1 : i-1;
...
void do_step_wolff(bool spins[SIZE],  int neighs[SIZE][4], double tstar, int N, double& energy, mt19937& gen, uniform_real_distribution<double>& ran_u, uniform_int_distribution<int>& ran_pos, double m[DATA])
{
	...
	//#pragma omp parallel for private(i,j) reduction(+:old_sum,old_chi,old_heat,old_energy)
	for (i=0; i < N; i++)
	{
		//Make changes and then average
        for (j=0; j < 12; j++)
		{
		 	add_to_cluster(spins, neighs, ran_pos(gen), energy, pa, gen, ran_u);
...
double get_energy(bool spins[SIZE], int neighs[SIZE][4])
{
    ...
    //For every spin,
    #pragma omp parallel for private(i,sum_neigh) reduction(+:energy)
    for (i=0; i < SIZE; i++)
    {
...
void w_output(ofstream& file, double tstar, int N, double m[DATA])
{
    file << tstar << "   " << 1.0/tstar << "   "; //Write T (temperature) and B (1/temperature)
